...
__global__ void ConvLayerBackward_Kernel(
	...
	float l_c1_b_preact = 0;
	float l_c1_b_weight = 0;
	...
	l_c1_b_preact = l_c1_b_output[m][x][y] * sigmoidPrime(l_c1_preact[m][x][y]);

	int i, j;
	for(i=0; i<5; i++){
		for(j=0; j<5; j++){
			l_c1_b_weight += l_c1_b_preact * l_input_output[x + i][y + j] / (24*24);
		}
	}

	l_c1_bias[m] += lr * l_c1_b_preact / (6*24*24);

	if(m==0 && x<5 && y<5)
		l_c1_weight[m][x][y] += lr * l_c1_b_weight;
}
...
__global__ void PoolLayerBackward_Kernel(
	...
	l_p_b_preact = l_p_b_output[x][y][z] * sigmoidPrime(l_p_preact[x][y][z]);

	l_p_bias[0] += lr * l_p_b_preact/(6*6*6);
	...
	for(i=0; i<4; i++) {
		for(j=0; j<4; j++) {
			...
			atomicAdd(&l_p_b_weight[0][i][j], l_p_b_preact * l_c1_output[x][y*4+i][z*4+j]);
			atomicAdd(&l_c1_b_output[x][y*4+i][z*4+j], l_p_weight[0][i][j] * l_p_b_preact);
		}
	}
	__syncthreads();
...
__global__ void FullyConLayerBackward_kernel(
	...
	l_f_b_weight = l_f_b_preact[m] * l_p_output[x][y][z];
	...
	l_f_weight[m][x][y][z] += lr * l_f_b_weight;
}
